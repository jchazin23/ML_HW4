...
for t in range(T):
    for i in range(m):
        for j in range(n):
            u[i] = -(np.dot(v[:,j],v[:,j].T)**-1) * (-(b[i] + mu - a_i_j[i,j]))*v[:,j]
            b[i] = -(np.dot(u[i],v[:,j]) + c[:,j] + mu - a_i_j[i,j])/n
    for j in range(n):
        for i in range(m):
            v[:,j] = -(np.dot(u[i],u[i].T)**-1) * (-(b[i] + mu - a_i_j[i,j]))*u[i]
            c[:,j] = -(np.dot(u[i],v[:,j]) + b[i] + mu - a_i_j[i,j])/m
